...
http_error_t http_request_parse(http_request_t* req, const char* line) {
	http_error_t err = HTTP_ERR_SYNTAX;
	...
	err = HTTP_ERR_NONE;
	req->version.major = line[5] - '0';
	req->version.minor = line[7] - '0';
...
void http_body_append(http_body_t* body, void* data, size_t len) {
	body->data = realloc(body->data, body->len + len);
	...
	while(!close) {
		...
		if(!line) {
			if(errno == ECONNRESET) {
				...
				close = true;
				onError(HTTP_ERR_SYNTAX, sockfd);
				goto end;
			}
		...
		http_error_t err = http_request_parse(&request, line);
		free(line);
		...
		while(line && strcmp(line, "\r\n") != 0) {
			...
			err = http_header_parse(line, &name, &value);
			if(err) {
				free(line);
		...
		char* hv = http_header_get(&request.headers, "Content-Length");
		if(hv) {
			int sz = atoi(hv);
			if(sz > HTTP_MAX_REQUEST_SIZE) {
				if(onError) {
					onError(HTTP_ERR_TOO_LARGE, sockfd);
				}
		...
		char* resStr = 0;
		err = http_response_format(&response, &resStr);
		if(err) {
			close = true;
...
/*
 * Return a string corresponding to an http_error_t object
 */
const char* http_error_str(http_error_t err) {
	switch(err) {
		...
		case HTTP_ERR_SYNTAX:
			return "invalid http message syntax";
		case HTTP_ERR_TOO_LARGE:
			return "http content too large";
		default:
